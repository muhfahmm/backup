...
export default function BeliModalsMenu({ isOpen, onClose, countryDetail, setCountryDetail, onConfirm }: BeliModalsMenuProps) {
  ...
  const komoditasImpor = [
    { nama: "Bahan Baku Elektronik", harga: 28000000, satuan: "1x Kontainer" },
    { nama: "Mesin Industri", harga: 45000000, satuan: "1x Unit" },
    { nama: "Bahan Bakar Minyak", harga: 18000000, satuan: "1x Tanker" }
  ];

  const handleBeli = (nama: string, harga: number, satuan: string) => {
    if (anggaran < harga) {
      ...
      <div className="bg-[#FAF6EE] border-4 border-[#C4B49C] rounded-2xl w-full max-w-2xl max-h-[80vh] overflow-hidden shadow-2xl flex flex-col relative font-sans">
        ...
        <div className="flex-1 overflow-y-auto p-6 relative z-10 space-y-3 no-scrollbar">
          <p className="text-xs text-[#8b7e66] font-semibold leading-relaxed mb-4">
            Pilih komoditas yang ingin diimpor dari pasar internasional. Biaya akan langsung dipotong dari Kas Negara.
          </p>
          ...
          {komoditasImpor.map((item, idx) => (
            ...
            >
              ...
              <button
                onClick={() => handleBeli(item.nama, item.harga, item.satuan)}
                className="px-5 py-2 rounded-lg bg-emerald-700 hover:bg-emerald-800 active:bg-emerald-900 text-white text-[10px] font-black uppercase tracking-wide cursor-pointer transition-all shadow-sm whitespace-nowrap"
              >
