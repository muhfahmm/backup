...
export default function PertahananModal({ isOpen, onClose, countryDetail, setCountryDetail }: ModalProps) {
  ...
  const handleMobilize = () => {
    if (anggaran < 30000000) {
      alert("Kas militer tidak mencukupi!");
      return;
    }
    setCountryDetail({
      ...countryDetail,
    ...
    alert("Berhasil memobilisasi resimen infantri mekanis tempur baru!");
  };
      ...
      <div className="bg-[#FAF6EE] border-4 border-[#C4B49C] rounded-2xl w-full max-w-6xl h-[84vh] overflow-hidden shadow-2xl flex flex-col relative font-sans">
        ...
        <div className="flex-1 min-h-0 overflow-y-auto p-8 bg-[#FAF6EE]/40 relative z-10 no-scrollbar">
          <p className="text-xs text-[#8b7e66] font-semibold leading-relaxed mb-6">
            Tinjau status siaga kedaulatan wilayah NKRI. Rekrut resimen tempur infantri baru untuk memperkuat hegemoni militer.
          </p>
          ...
          <div className="bg-[#e4dac3]/20 border border-[#C4B49C]/30 p-4 rounded-xl mb-6">
            <div className="flex justify-between text-xs font-bold text-[#5c3c10] mb-2">
              ...
              <span>12 Batalyon Infantri</span>
            </div>
