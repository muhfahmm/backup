...
interface ModalProps {
  ...
  // 🔥 Callback untuk aksi serang
  onAttack?: (targetCountry: any) => void;
}
...
export default function SerangNegaraModal({ 
  ...
  // Fungsi logic sorting interaktif
  ...
  // 🔥 Fungsi untuk menyerang target secara langsung (tanpa seleksi baris)
  const handleDirectAttack = (target: RankingRow) => {
    if (onAttack) {
      onAttack(target);
    } else {
      alert(`Anda memilih untuk menyerang: ${target.countryName}`);
    }
    onClose();
  };
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-transparent pointer-events-none">
      <div className="bg-[#FAF6EE] border-4 border-[#C4B49C] rounded-2xl w-full max-w-7xl h-[84vh] overflow-hidden shadow-2xl flex flex-col relative font-sans pointer-events-auto">
                  ...
                  {rankings === null ? (
                    ...
                    rankings.map((row, index) => (
                      ...
                      >
                        ...
                        {/* 🔥 Tombol aksi serang langsung */}
                        <td className="px-3 py-2 text-center">
                          <button 
                            onClick={() => handleDirectAttack(row)}
        ...
        {/* 🔥 FOOTER MODAL - Hanya tombol Batal */}
