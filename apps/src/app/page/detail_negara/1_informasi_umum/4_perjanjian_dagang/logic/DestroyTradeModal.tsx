...
interface DestroyTradeModalProps {
  isOpen: boolean;
...
export default function DestroyTradeModal({ isOpen, countryName, onClose, onConfirm }: DestroyTradeModalProps) {
  if (!isOpen) return null;

  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40">
      <div className="w-[420px] bg-white rounded-2xl p-6 shadow-lg border border-[#E5DCCF]">
        <h3 className="text-lg font-black text-[#3d2911] mb-3">Konfirmasi Putus Hubungan Dagang</h3>
        <p className="text-sm text-[#5c3c10] mb-6">Apakah Anda yakin ingin memutus hubungan dagang dengan <strong>{countryName}</strong>? Anda dapat menjalin kembali melalui menu Mitra.</p>
