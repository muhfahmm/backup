...
#[wasm_bindgen]
pub async fn start_map_engine(canvas_id: String) -> Result<(), JsValue> {
    ...
    wasm_bindgen_futures::spawn_local(async move {
        ...
        let world_fut = wasm_bindgen_futures::JsFuture::from(window.fetch_with_str("/data/world.json"));
        let countries_fut = wasm_bindgen_futures::JsFuture::from(window.fetch_with_str("/data/countries.json"));
        let capitals_fut = wasm_bindgen_futures::JsFuture::from(window.fetch_with_str("/data/capitals.json"));
        
        if let (Ok(world_res), Ok(countries_res), Ok(capitals_res)) = (world_fut.await, countries_fut.await, capitals_fut.await) {
            let world_res: Response = world_res.dyn_into().unwrap();
            let countries_res: Response = countries_res.dyn_into().unwrap();
            let capitals_res: Response = capitals_res.dyn_into().unwrap();
            
            let world_text = wasm_bindgen_futures::JsFuture::from(world_res.text().unwrap()).await.unwrap().as_string().unwrap();
            let countries_json = wasm_bindgen_futures::JsFuture::from(countries_res.json().unwrap()).await.unwrap();
            let capitals_json = wasm_bindgen_futures::JsFuture::from(capitals_res.json().unwrap()).await.unwrap();
            
            let mut engine = engine_clone.borrow_mut();
            engine.set_data(&world_text);
            engine.set_countries(countries_json.dyn_into::<js_sys::Array>().unwrap().to_vec());
            engine.set_capitals(capitals_json.dyn_into::<js_sys::Array>().unwrap().to_vec());
        }
    });
