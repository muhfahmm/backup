...
export default function NavAtasUtama({ isActive, onClick }: NavAtasProps) {
  return (
    <button
      onClick={onClick}
      className={`
        ...
        ${isActive 
          ? 'bg-white/10 shadow-[0_0_15px_rgba(34,211,238,0.3)] border border-white/10 px-4 py-2 gap-2.5' 
    ...
    >
      <Globe 
        size={16} 
        className={`transition-all duration-300 ${isActive ? 'text-cyan-400' : 'text-zinc-500 group-hover:text-zinc-300'}`} 
      />
      ...
      {!isActive && <Tooltip label="Map Utama" />}
    </button>
  );
}
